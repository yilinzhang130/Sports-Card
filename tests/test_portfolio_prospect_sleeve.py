...
def _seed_player_with_rcs(s, *, name, slug, draft_year, premium, sets):
    ...
    s.add(
        PlayerStardomScore(
            ...
            fit_at=datetime(draft_year, 12, 1, tzinfo=UTC),
        )
    )
    s.commit()
    return p, cards
...
def test_load_stardom_picks_prizm_silver_when_available(session):
    ...
    df = load_stardom(session, as_of=datetime(2026, 1, 1, tzinfo=UTC))
    assert df is not None
    silver_id = next(c.card_id for c in cards if c.set_name == "Prizm" and c.parallel == "Silver")
...
def test_load_stardom_excludes_outside_rookie_window(session):
    ...
    df = load_stardom(session, as_of=datetime(2025, 6, 1, tzinfo=UTC))
    assert df is not None
...
def test_load_stardom_returns_none_when_empty(session):
    assert load_stardom(session, as_of=datetime(2026, 1, 1, tzinfo=UTC)) is None
...
def test_load_stardom_returns_none_when_table_missing():
    ...
    s = Session(eng)
    try:
        assert load_stardom(s, as_of=datetime(2026, 1, 1, tzinfo=UTC)) is None
    finally:
        s.close()
