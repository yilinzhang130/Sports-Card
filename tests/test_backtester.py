...
def test_flat_prices_zero_fees_round_trip_neutral() -> None:
    ...
    zero_fees = FeeSchedule(
        ebay_pct=0.0, ebay_flat=0.0, auction_buyer_premium_pct=0.0,
        auction_seller_commission_pct=0.0,
    )
    cfg = BacktestConfig(
        start=start, end=end, fee_schedule=zero_fees,
    ...
    class _Stub:
        ...
        def execute(self, *_a, **_kw):
            class _R:
                ...
                def all(self_inner):
                    return [
                        {"card_id": 1, "mispricing_residual": 0.1,
                         "computed_at": pd.Timestamp("2030-01-01"),
                         "sport": "NBA", "parallel_tier": "base"},
                    ]
            return _R()
