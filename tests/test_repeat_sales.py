...
def test_recovers_latent_index_from_synthetic_data() -> None:
    ...
    merged = idx.dropna(subset=["index_value"]).merge(truth, on="period_start", how="inner")
    ...
    corr = np.corrcoef(log_recovered - log_recovered.mean(), log_truth - log_truth.mean())[0, 1]
...
def test_chain_stability_adding_new_pairs_does_not_change_history() -> None:
    ...
    common = idx_early.merge(idx_full, on="period_start", suffixes=("_early", "_full")).dropna(
        subset=["index_value_early", "index_value_full"]
    )
...
def test_same_period_pairs_are_filtered() -> None:
    ...
    tx = pd.DataFrame(
        [
            ...
            {
                "cert_number": "A",
                "sold_at": base + pd.Timedelta(days=2),
                "price_usd": 110.0,
                "slab_grader": "PSA",
                "slab_grade": 10.0,
            },
            ...
            {
                "cert_number": "B",
                "sold_at": base + pd.Timedelta(weeks=30),
                "price_usd": 260.0,
                "slab_grader": "PSA",
                "slab_grade": 10.0,
            },
