...
ExitRule = Literal["target_hit", "factor_reversal", "time_stop", "price_stop", "liquidity_degrade"]
...
def evaluate_open_positions(session: Session, as_of: date) -> list[ExitSignalDTO]:
    holdings = (
        session.execute(select(PortfolioHolding).where(PortfolioHolding.status == "held"))
        .scalars()
        .all()
    )
    if not holdings:
...
def _targets_lookup(session: Session, as_of: date) -> dict[int, TradeTargets]:
    rows = (
        session.execute(select(TradeTargets).where(TradeTargets.as_of_date == as_of))
        .scalars()
        .all()
    )
...
def _panel_lookup(session: Session, as_of: date) -> dict[int, FactorPanel]:
    ...
    rows = (
        session.execute(select(FactorPanel).where(FactorPanel.as_of_date == as_of_ts))
        .scalars()
        .all()
    )
    return {int(r.card_id): r for r in rows}
