...
def compute_stardom_premium(
    ...
    """
    # All inputs forced to positional ndarrays so a mismatched Series index
    # cannot silently re-align columns under us. Length mismatch raises.
    df = pd.DataFrame(
        {
            "br_slug": np.asarray(br_slugs),
            "draft_year": pd.to_numeric(np.asarray(draft_years), errors="coerce"),
            "draft_pick": pd.to_numeric(np.asarray(draft_picks), errors="coerce"),
            "model_score": np.asarray(model_scores),
        }
    )
    max_pick = df["draft_pick"].max()
    if not np.isfinite(max_pick):
        max_pick = 60.0
...
def persist_scores(
    ...
    for row in scores.itertuples(index=False):
        ...
        if pid is None:
            skipped_unmapped += 1
            continue
        if not pd.notna(row.draft_year):
            skipped_bad_year += 1
            continue
        session.add(
            PlayerStardomScore(
        ...
        n += 1
    if skipped_unmapped or skipped_bad_year:
        logger.warning(
            "persist_scores skipped %d unmapped slugs, %d rows with missing draft_year",
            skipped_unmapped,
            skipped_bad_year,
        )
    return n
